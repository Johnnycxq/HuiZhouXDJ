...
/**
 * Created by ruihong on 2018/5/4.
 */

public class LoginVpnActivity extends BaseCheckPermissionActivity implements LoginResultListener, RandCodeListener {
    ...
    private String mVpnAddress = "https://59.33.254.90";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mSFManager.ticketAuthAvailable(this)) { //允许免密，直接走免密流程
            try {
                ...
                mSFManager.startTicketAuthLogin(getApplication(), LoginVpnActivity.this, mVpnMode);
            } catch (SFException e) {
                //关闭登录进度框
    ...
    private void initClickEvents() {
        ...
        mTicketLoginButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Toast.makeText(LoginVpnActivity.this, R.string.str_open_bypass, Toast.LENGTH_SHORT).show();
            }
        });
        ...
        mAddWhitePackageButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (TextUtils.isEmpty(packageName)) {
                    Toast.makeText(LoginVpnActivity.this, R.string.str_package_empty, Toast.LENGTH_SHORT).show();
                    return;
                } else {
                    Toast.makeText(LoginVpnActivity.this, R.string.str_add_package_success, Toast.LENGTH_SHORT).show();
                }
                //l3vpn模式下，添加应用白名单
    ...
    private void addStatusChangedListener() throws SFException {
        mSFManager.addStatusChangedListener(new OnStatusChangedListener() {
            ...
            public void onStatusCallback(IVpnDelegate.VPNStatus vpnStatus, StatusChangedReason reason) {
                ...
                Toast.makeText(LoginVpnActivity.this, status, Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private boolean getValueFromView() {
        ...
        if (TextUtils.isEmpty(authMethod)) {
            Toast.makeText(LoginVpnActivity.this, R.string.str_auth_type_select_error, Toast.LENGTH_SHORT).show();
            return false;
        } else {
        ...
        if (TextUtils.isEmpty(mVpnAddress)) {
            Toast.makeText(LoginVpnActivity.this, R.string.str_vpn_address_is_empty, Toast.LENGTH_SHORT).show();
            return false;
        }
        ...
        try {
            if (!mVpnAddress.startsWith("https://")) { //vpn地址是否是以https开头，不以https开头时，为其添加https
                ...
                if (index == -1) {//没有协议头的情况下添加https协议头
                    ...
                    //其它协议头提示用户错误
                    Toast.makeText(LoginVpnActivity.this, R.string.str_url_protocol_error, Toast.LENGTH_SHORT).show();
                    return false;
                }
            }
            //将地址字符串封装成url
            ...
            Toast.makeText(LoginVpnActivity.this, R.string.str_url_error, Toast.LENGTH_SHORT).show();
            return false;
        }
        ...
        try {
            ...
            switch (mAuthMethod) {
                case AUTH_TYPE_PASSWORD:
                    ...
                    mSFManager.startPasswordAuthLogin(getApplication(), LoginVpnActivity.this, mVpnMode,
                            mVpnAddressURL, mUserName, mUserPassword);
                    break;
                case AUTH_TYPE_CERTIFICATE:
                    //该接口做了两件事：1.vpn初始化；2.证书主认证过程
                    mSFManager.startCertificateAuthLogin(getApplication(), LoginVpnActivity.this, mVpnMode,
                            mVpnAddressURL, mCertPath, mCertPassword);
                    break;
                default:
                    Toast.makeText(LoginVpnActivity.this, R.string.str_auth_type_error, Toast.LENGTH_SHORT).show();
                    break;
            }
        } catch (SFException e) {
            //关闭登录进度框
            ...
            switch (authType) {
                ...
                case AUTH_TYPE_RENEW_PASSWORD:
                    ...
                    if (newPwd.equals(reNewPwd)) {
                        ...
                        cancelWaitingProgressDialog();
                        Toast.makeText(LoginVpnActivity.this, R.string.str_password_not_same, Toast.LENGTH_SHORT).show();
                        return;
                    }
                    break;
    ...
    public View createDialogView(int aythtype, int layoutId, BaseMessage message) {
        ...
        switch (aythtype) {
            ...
            case AUTH_TYPE_RAND_CODE:
                ...
                try {
                    mSFManager.setRandCodeListener(LoginVpnActivity.this);
                } catch (SFException e) {
                    Log.info(TAG, "SFException:%s", e);
                }
    ...
    protected void createWaitingProgressDialog() {
        if (mProgressDialog == null || !mProgressDialog.isShowing()) {
            mProgressDialog = new ProgressDialog(LoginVpnActivity.this);
    ...
    protected void permissionGrantedFail() {
        Toast.makeText(LoginVpnActivity.this, R.string.str_permission_not_all_pass, Toast.LENGTH_SHORT).show();
